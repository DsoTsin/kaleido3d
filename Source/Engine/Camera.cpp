...
namespace k3d {
	...
	void Camera::SetProjection(float fovy, float ratio, float near_plane, float far_plane)
	{
	...
	const kMath::Mat4f Camera::GetViewMatrix() const
	{
		return m_ViewMatrix;
	}

	const kMath::Mat4f Camera::GetProjectionMatrix() const
	{
		return m_ProjectionMatrix;
	}

	void Camera::SetPosition(const kMath::Vec3f &v)
	{
		m_CameraPosition = v;
	}

	const kMath::Vec3f Camera::GetPosition() const
	{
	...
	bool Camera::IsPointInFrustum(const kMath::Vec3f &point)
	{
		for (int i = 0; i < 6; i++)
		{
	...
	bool Camera::IsSphereInFrustum(const kMath::BoundingSphere &sphere)
	{
		for (int i = 0; i < 6; i++)
		{
	...
	BoundType Camera::IntersectBox(const kMath::AABB &aabb)
	{
	...
	void Camera::GetFrustumPlanes(kMath::Vec4f planes[])
	{
		for (int i = 0; i < 6; i++)
		{
