...
void kOctreeSceneManager::SetMaxTreeDepth(const uint32 &_MaxTreeDepth)
{
  m_MaxTreeDepth = std::min<uint32>(_MaxTreeDepth, 16u);
}

const uint32 kOctreeSceneManager::GetMaxTreeDepth() const
{
  return m_MaxTreeDepth;
}

void kOctreeSceneManager::ClipScene()
{
  if(m_RebuildTree)
  {
    m_Octree.resize(1);
    ...
    for(kSObject::SObjPtr &obj : m_SceneObjs)
    {
      uint32 type = obj->GetType();
      if( type == k3dRenderNode::Node_Object )
      {
...
void kOctreeSceneManager::AddSceneObject(const kSObject::SObjPtr & obj)
{
//  AABBoxPtr aabb_ws;
//  uint32_t const attr = obj->Attrib();
//  if ((attr & SceneObject::SOA_Cullable)
...
void kOctreeSceneManager::DeleteSceneObject()
{

}

void kOctreeSceneManager::SplitNode(int index, uint32 curNodeDepth)
{
  if (m_Octree[index].ObjPtrs.size() > 1)
  {
    ...
    for(kSObject::SObjPtr const & soaabb : m_Octree[index].ObjPtrs)
    {
    ...
    for (size_t j = 0; j < 8; ++ j)
    {
      ...
      {
        this->SplitNode(this_size + j, curNodeDepth + 1);
      }
    }

    kSObject::SOVector empty;
    m_Octree[index].ObjPtrs.swap(empty);
  }
}

void kOctreeSceneManager::NodeVisible(int index)
{
//  bool visible = true;
...
void kOctreeSceneManager::MarkNodeObjs(int index, bool force)
{
  assert(index < m_Octree.size());
  kOctreeNode const & node = m_Octree[index];
  if ((node.Visible != BO_NO) || force)
  {
    for(kSObject::SObjPtr const & obj : node.ObjPtrs)
    {
...
bool kOctreeSceneManager::BoundVisible(int index, const AABB &aabb) const
{
  K3D_UNUSED(index);
  K3D_UNUSED(aabb);
  return true;
