...
void kFreeCamera::Walk(const float dt)
{
  m_Translation += (m_LookVector * m_CameraSpeed * dt);
  Update();
}

void kFreeCamera::Strafe(const float dt)
{
  m_Translation += (m_RightVector*m_CameraSpeed*dt);
  Update();
}

void kFreeCamera::Lift(const float dt)
{
  m_Translation += (m_UpVector*m_CameraSpeed*dt);
  Update();
}

void kFreeCamera::SetTranslation(const kMath::Vec3f &t)
{
  m_Translation = t;
  Update();
}

kMath::Vec3f kFreeCamera::GetTranslation() const
{
  return m_Translation;
}

void kFreeCamera::SetSpeed(const float speed)
{
  m_CameraSpeed = speed;
}

const float kFreeCamera::GetSpeed() const
{
