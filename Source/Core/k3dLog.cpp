...
bool k3dLog::InitLogFile( const char *name )
{
  if ( g_LogFile==NULL )
    g_LogFile = GetIODevice<k3dFile>();
...
void k3dLog::Message( const char *fmt, ... )
{
  std::unique_lock<std::mutex> lock( g_LogLock );
  va_list va;
  va_start( va, fmt );
...
void k3dLog::Warning( const char *fmt, ... )
{
  std::unique_lock<std::mutex> lock( g_LogLock ); 
  va_list va;
  va_start( va, fmt );
...
void k3dLog::Error( const char *fmt, ... )
{
  std::unique_lock<std::mutex> lock( g_LogLock ); 
  va_list va;
  va_start( va, fmt );
...
void k3dLog::Fatal( const char *fmt, ... )
{
  std::unique_lock<std::mutex> lock( g_LogLock ); 
  va_list va;
  va_start( va, fmt );
