...
public:
  ...
  template <typename T>
  k3dArchive & operator << (const T data) {
    assert(std::is_pointer<T>::value!=true && "cannot be serialize, not a pod class!!");
...
#if 0
class kMemoryArchiveReader : public k3dArchive {
public:
...
class kMemoryArchiveWriter : public k3dArchive {
public:
