...
std::string DumpSubmitInfo(VkSubmitInfo info)
{
	...
	{
		for (uint32 i = 0; i < info.waitSemaphoreCount; ++i)
		{
	...
	{
		for (uint32 i = 0; i < info.commandBufferCount; ++i)
		{
	...
	{
		for (uint32 i = 0; i < info.signalSemaphoreCount; ++i)
		{
...
#define DYNVKLDLOG(x) VkLog(k3d::ELogLevel::Debug, "DynVkLoader", (x));
...
std::string DumpGraphicsPipelineCreateInfo(VkGraphicsPipelineCreateInfo gInfo)
{
	...
	if (gInfo.stageCount)
	{
		for (uint32 i = 0; i < gInfo.stageCount; i++)
		{
...
std::string DumpBufferCopy(VkBufferCopy BufCpy)
{
	std::stringstream param;
	param << "{ SrcOffSet=" << BufCpy.srcOffset << ", DestOffSet=" << BufCpy.dstOffset << ", Size=" << BufCpy.size << " }";
	return param.str();
}
...
void vkCmd::CopyBuffer(VkCommandBuffer commandBuffer, VkBuffer srcBuffer, VkBuffer dstBuffer, uint32_t regionCount, const VkBufferCopy * pRegions)
{
	std::stringstream param;
	param << "vkCmd::CopyBuffer() commandBuffer=" << std::hex << std::setfill('0') << commandBuffer
		<< "\n\t,SrcBuffer=" << std::hex << std::setfill('0') << srcBuffer
		<< "\n\t, DestBuffer=" << std::hex << std::setfill('0') << dstBuffer
		<< "\n\t, RegionCount=" << std::dec << regionCount;
	if (regionCount)
	{
		for (uint32 i = 0; i < regionCount; i++)
			param << "\n\t, BufferCopies[" << i << "]=" << DumpBufferCopy(pRegions[i]);
	}
	DYNVKLDLOG(param.str().c_str());
	param << "\n";
