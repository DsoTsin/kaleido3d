...
public:
	...
	VkRenderPass const &		GetTopPass() const { return m_PendingPass.back(); }
	void						PushRenderPass(VkRenderPass const & renderPass) { m_PendingPass.push_back(renderPass); }

private:

	PtrResManager						m_ResourceManager;
	std::unique_ptr<CommandContextPool>	m_ContextPool;
	std::vector<VkRenderPass>			m_PendingPass;
