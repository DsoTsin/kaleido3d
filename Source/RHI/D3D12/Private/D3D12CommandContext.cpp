...
#if _DEBUG
	DX12LOG(Info,"CommandContext Allocated.");
#endif
}
...
{
	m_CommandList->CopyBufferRegion(static_cast<GpuResource&>(Dest).GetResource(), Region.DestOffSet, static_cast<GpuResource&>(Src).GetResource(), Region.SrcOffSet, Region.CopySize);
}
...
void CommandContext::SetRenderTarget(rhi::RenderTargetRef)
{
}
...
void CommandContext::ClearColorBuffer(rhi::GpuResourceRef, kMath::Vec4f const & )
{
}
...
void CommandContext::SetPipelineState(uint32 Hash, rhi::PipelineStateObjectRef RhiPipeLineState)
{
	auto pPSO = k3d::StaticPointerCast<PipelineState>(RhiPipeLineState);
	K3D_ASSERT(pPSO);
...
{
	auto pipelineLayout = k3d::StaticPointerCast<PipelineLayout>(pRHIPipelineLayout);
...
void CommandContext::TransitionResourceBarrier(rhi::GpuResourceRef resource, rhi::EResourceState dstState)
{
}
